...
## Modül 5: Kapsamlı Raporlama ve Bilgi Sentezi

Bu modül, Modül 4'ten gelen zenginleştirilmiş makale listesini işleyerek, kullanıcının orijinal prompt'u etrafında anlamlı bir anlatı veya özet oluşturur. Modül 5'in temelinde, detaylı makale analizlerini sentezleyerek kapsamlı ve sentezlenmiş bir rapor sunmak yatar.

### 5.1 Gerekli Veriyi Toplama ve Hazırlama

Modül 5, Modül 4'ten gelen zenginleştirilmiş makale listesini işler. Bu, her bir URL için detaylı analiz sonuçlarını içeren bir dizi olacaktır. Her makale için şu bilgiler mevcuttur:

- **Temel Bilgiler**: URL, başlık, içerik, özet, yazar, yayın tarihi
- **Analiz Sonuçları**: Alaka skoru, duygu analizi, tarafsızlık analizi, okunabilirlik skorları
- **Metin Metrikleri**: Kelime sayısı, cümle sayısı, karakter sayısı, kelime çeşitliliği

### 5.2 Rapor Çıkarma Adımları (Algoritma Akışı)

#### Adım 1: Makaleleri Filtreleme ve Sıralama
- **Relevance ve Zaman Kısıtı**: `is_relevant` ve `is_within_time_limit` bayraklarını kullanarak sadece alakalı ve güncel makaleleri seç
- **Sıralama**: Makaleleri `relevance_score`'a göre azalan şekilde sırala
- **max_results Uygulama**: Kullanıcının belirttiği `max_results` parametresine göre en iyi N sayıda makaleyi seç

#### Adım 2: Önemli İçgörüleri Çıkarma ve Sentezleme

**Ana Konu Tespiti ve Özet (Genel Rapor Özeti):**
- Seçilen en alakalı makalelerin içeriklerini birleştir
- Hugging Face Transformers kütüphanesindeki özetleme modelini kullan
- "Verilen makalelerin ana konularını ve en önemli bulgularını özetleyen kısa bir rapor oluştur" prompt'u ile rehberlik et

**Duygu Analizi Sentezi:**
- Tüm seçilen makalelerin `sentiment_analysis` sonuçlarını topla
- Genel bir ortalama duygu skoru veya baskın duygu belirle
- "Analiz edilen makalelerin çoğunluğu pozitif bir tona sahip" gibi yorumlar oluştur

**Tarafsızlık/Sübjektiflik Yorumu:**
- Her makalenin `objectivity_analysis` sonuçlarını incele
- Makalelerin genel olarak ne kadar tarafsız veya sübjektif olduğunu yorumla

**Anahtar Oyuncular/Varlıklar:**
- Seçilen makalelerin içeriklerinden Named Entity Recognition (NER) kullanarak önemli şirket adlarını, kişi adlarını veya anahtar kavramları çıkar
- "Konuyla ilgili öne çıkan şirketler/kişiler: X, Y, Z" bölümü oluştur

**Ortak Temalar ve Trendler:**
- Makalelerin özetlerini kullanarak ortak temaları veya tekrar eden kavramları tespit et
- "Analiz edilen makalelerde sıkça vurgulanan temalar arasında [tema 1], [tema 2] ve [tema 3] bulunmaktadır" şeklinde özetle

#### Adım 3: Nihai Rapor Formatı ve Çıktı
Tüm sentezlenmiş bilgileri kullanıcı dostu bir JSON yapısında bir araya getir ve aynı zamanda Markdown formatında da sun.

### 5.3 Nihai Rapor Formatı ve Çıktı

#### JSON Çıktı Formatı
```json
{
  "query": "Yapay zeka etiği ve şeffaflık konularındaki son gelişmeler",
  "report_summary": "Yapay zeka etiği ve şeffaflık alanındaki son gelişmeler, hem teknolojik ilerlemeleri hem de bu teknolojilerin toplumsal etkilerini ele almaktadır. Özellikle büyük dil modellerinin etik kullanımı, veri gizliliği ve algoritmik tarafsızlık konuları öne çıkmaktadır. İncelenen kaynaklar genellikle bu gelişmelerin potansiyelini olumlu bir tonda ele alırken, etik zorluklara da dikkat çekmektedir.",
  "overall_sentiment": {
    "dominant_sentiment": "positive",
    "details": "Analiz edilen makalelerin %70'i pozitif, %20'si nötr ve %10'u hafif negatif bir duygu tonu taşımaktadır."
  },
  "objectivity_overview": "Çoğu makale nesnel bilgi sunmakla birlikte, bazı yorum tabanlı makalelerde sübjektif ifadeler de bulunmaktadır.",
  "key_entities_mentioned": ["OpenAI", "Anthropic", "Google DeepMind", "AI Ethics Institute"],
  "trending_themes": ["algoritma şeffaflığı", "sorumlu yapay zeka", "veri gizliliği", "yapay zeka düzenlemeleri"],
  "most_relevant_articles": [
    {
      "url": "https://techcrunch.com/example-ai-ethics-article-1",
      "title": "Yeni AI Etik Çerçeveleri...",
      "summary": "Bu makale, yapay zeka sistemlerinin geliştirilmesinde şeffaflığı artırmayı amaçlayan en son etik çerçeveleri detaylandırıyor.",
      "author": "Yazar Adı",
      "publish_date": "2025-07-20T...",
  ...
  "processing_details": {
    "total_articles_scraped": 50,
    "articles_analyzed": 20,
    "articles_included_in_report": 5,
...
Aynı zamanda kullanıcı dostu bir Markdown raporu da oluşturulur:

```markdown
# Yapay Zeka Etiği ve Şeffaflık Raporu

## Özet
Yapay zeka etiği ve şeffaflık alanındaki son gelişmeler...

## Genel Duygu Analizi
Analiz edilen makalelerin %70'i pozitif, %20'si nötr ve %10'u hafif negatif bir duygu tonu taşımaktadır.

## Tarafsızlık Değerlendirmesi
Çoğu makale nesnel bilgi sunmakla birlikte...
...
## En Alakalı Makaleler

### 1. Yeni AI Etik Çerçeveleri
**Kaynak:** https://techcrunch.com/example-ai-ethics-article-1  
**Yazar:** Yazar Adı  
**Tarih:** 20 Temmuz 2025  
**Alaka Skoru:** 0.95  
**Duygu:** Pozitif  
**Okunabilirlik:** 10.5. sınıf seviyesi

Bu makale, yapay zeka sistemlerinin geliştirilmesinde şeffaflığı artırmayı amaçlayan en son etik çerçeveleri detaylandırıyor...

## İşlem Detayları
- **Toplam taranan makale:** 50
- **Analiz edilen makale:** 20
- **Rapora dahil edilen makale:** 5
- **Toplam işlem süresi:** 15 saniye
```

Bu modülün çıktısı, kullanıcının prompt'una hem anlamsal olarak alakalı olduğu onaylanmış hem de detaylı analizlerle zenginleştirilmiş, kapsamlı ve sentezlenmiş bir rapor olacaktır. Bu rapor hem JSON hem de Markdown formatında sunulur.
